// screens/results.js
import { getStatutGlobal, STATUT_COLOR } from '../db.js';
...
async function enterResults(state) {
  ...
  if (!errors.length) {
    ...
    document.getElementById('results-retry').style.display = 'none';
    return;
  }

  document.getElementById('results-retry').style.display = '';
  ...
  // Sauvegarder state pour retry
  document.getElementById('results-retry')._state = state;
}

function retryErrors() {
  const state = document.getElementById('results-retry')._state;
  navigate('screen-quiz', {
    cards:   state.errors,
    type:    state.params.type,
    sens:    state.params.sens,
    cat:     state.params.cat,
    critere: state.params.critere,
    listes:  state.params.listes,
  });
}
